...
public class CopyAndRenameTest {
	...
	// 文件夹的拷贝
	public static void copyDir(String sourceName, String targetName, String sourceDirName, String targetDirName,
			String sourcePath, String newPath) {
		...
		for (String temp : filePath) {
			...
			if (new File(sourcePath + File.separator + newTemp).isDirectory()) {
				// 为文件夹，进行递归
				copyDir(sourceName, targetName, sourceDirName, targetDirName, sourcePath + File.separator + temp,
						newPath + File.separator + newTemp);
			}
			else {
				// 为文件则进行拷贝
				copyFile(sourceName, targetName, sourceDirName, targetDirName, sourcePath + File.separator + temp,
						newPath + File.separator + newTemp);
			}
		}
	}
	...
	// 文件的拷贝
	public static void copyFile(String sourceName, String targetName, String sourceDirName, String targetDirName,
			String sourcePath, String newPath) {
		File start = new File(sourcePath);
		File end = new File(newPath);
