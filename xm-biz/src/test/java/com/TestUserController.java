...
public class TestUserController extends HttpTester {
	...
	public void test_002_GetById() throws Exception {
		HttpUtils httpUtils = path("/user/getById?id=user_1009").bodyJson("{}");
		httpUtils.post();
	}

	@Test
	public void test_003_Del() throws Exception {
		HttpUtils httpUtils = path("/user/deleteByIds").bodyJson("[\"user_1009\"]");
		httpUtils.post();
	}
...
//				+ "  \"createTime\": \"2023-06-29 11:49:40\",\n" + "  \"updateTime\": \"2023-06-29 13:32:46\",\n"
				...
				+ "  \"isSystemDefault\": 0,\n" + "  \"tenantId\": \"tenant1\",\n" + "  \"version\": 1,\n"
				+ "  \"status\": \"active\"\n" + "}\n";
		HttpUtils httpUtils = path("/user/add").bodyJson(data);
		httpUtils.post();
	}
	...
	public void test_005_Update() throws Throwable {
		final String data = "{\n" + "  \"userId\": \"user_1009\",\n" + "  \"userName\": \"我修改了名字"
				...
				// " \"status\": \"active\"\n" +
				"  \"status\": null\n" + "}\n";
		HttpUtils httpUtils = path("/user/update").timeout(5, -1, -1).bodyJson(data);
		httpUtils.post();
	}
