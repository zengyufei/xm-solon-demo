...
public class TreeUtil {
	...
	public static <T> List<T> build4rootField(List<T> list, String field) {
		...
		return build(list, l -> {
			...
			// 字段值为空的集合
			List<T> rootList = l.stream().filter(item -> {
	 ...
	 * @return 树机构数据集合
	 */
	public static <T> List<T> build(List<T> list, Function<List<T>, List<T>> getRootList,
	...
	public static <T> List<T> getChildren(T parent, List<T> list, TreeNodeConfig<T> nodeConfig, Integer deep) {
		// 校验
		if (Objects.isNull(parent) || Objects.isNull(list) || list.isEmpty() || Objects.isNull(nodeConfig)) {
			return new ArrayList<>();
		}
		...
		list.forEach(node -> {
			...
			// 设置该节点的子集合
			ReflectUtil.setFieldValue(node, nodeConfig.getChildrenKey(),
					getChildren(node, list, nodeConfig, Objects.isNull(deep) ? null : deep - 1));
			// 放入子集合
			children.add(node);
		});
	...
	public static <T> void push2list(List<T> list, T node, TreeNodeConfig<T> nodeConfig) {
		...
		// 是否有子集合
		if (Objects.isNull(nodeConfig) || Objects.isNull(nodeConfig.getChildrenKey())
		...
		children.forEach(item -> {
			if (Objects.nonNull(parentId) && Objects.nonNull(nodeConfig.getParentIdKey())
					&& !"".equals(nodeConfig.getParentIdKey().trim())) {
	 ...
	 * 树节点通用对比器
	 * <p>
	 * a, b 返回结果说明： -1: a小于b 0: a等于b 1: d大于b
	 *
	 * @param <T> 泛型
	 */
	@Data
