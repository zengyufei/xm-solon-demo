...
// * @return {@link EqualsTo}
// */
// public static EqualsTo getEqualsToString(final String alias, String fieldName, String
// value) {
...
// isExistsBuildUnitId[0] = StrUtil.equalsIgnoreCase(expr.getLeftExpression().toString(),
// fieldName);
// }
...
// }
//
// public static void setWhereIfAnd(Statement statement, Expression expression, boolean
// isPack) {
...
// DataAuthUtil.setWhereIfAnd(plainSelect, DataAuthUtil.getEqualsToString(null, "1",
// "2"));
// }
//
// public static void setNoAuthQuery(Statement statement) {
// DataAuthUtil.setWhereIfAnd(statement, DataAuthUtil.getEqualsToString(null, "1", "2"),
// false);
// }
//
// public static void setWhereIfAnd(PlainSelect plainSelect, Expression expression,
...
// }
//
// public static void setIn(PlainSelect plainSelect, String columnName, List<String>
// values) {
...
// 创建一个空的ExpressionList列表，用于存储后面需要构建的ExpressionList
//
...
// CollUtil.split(values, 1000).forEach(list -> expressionListList.add(new
// ExpressionList(XmUtil.listFiltersBlankDistinctMapToList(list, StringValue::new))));
// if (CollUtil.isNotEmpty(expressionListList)) { // 如果 expressionListList 不为空
// List<Expression> inExpressionList = expressionListList.stream() // 将 expressionListList
// 转换成 Expression 列表
// .map(expressionList -> new InExpression(new Column(columnName), expressionList)) //
// 针对每个 ExpressionList 新建 InExpression
// .collect(Collectors.toList()); // 将上一步操作得到的 InExpression 列表转换成 List<Expression> 类型
// expression = inExpressionList.stream().reduce(OrExpression::new).orElse(null); // 将
// InExpression 列表使用 OrExpression 进行合并
// }
// }
// return new Parenthesis(expression); // 返回构建完成的条件表达式加上圆括号
// }
//
// public static void setOr(PlainSelect plainSelect, Expression expression, boolean
// isPack) {
// final Expression plainSelectWhere = plainSelect.getWhere();
// // 与sql拼接
// if (null == plainSelectWhere) {
