...
public class PageImpl<T> extends Chunk<T> implements Page<T> {
    ...
    public PageImpl(List<T> content, Pageable pageable, long total) {
        ...
        this.total = pageable.toOptional().filter((it) -> !content.isEmpty()).filter((it) -> it.getOffset() + (long) it.getPageSize() > total).map((it) -> it.getOffset() + (long) content.size()).orElse(total);
    }
