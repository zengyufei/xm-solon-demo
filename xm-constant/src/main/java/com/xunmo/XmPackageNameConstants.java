package com.xunmo;

public interface XmPackageNameConstants {

	boolean IS_CONSOLE_LOG = true;

	String XM_CORE = "xm-core";

	String XM_POOL = "xm-pool";

	String XM_AUTO_TRAN = "xm-auto-tran";

	String XM_REQUEST_ARGS_CONSOLE = "xm-request-args-console";

	String XM_REQUEST_TIMES_CONSOLE = "xm-request-times-console";

	String XM_REQUEST_TRACE_ID = "xm-request-trace-id";

	String XM_REQUEST_XSS_FILTER = "xm-request-xss-filter";

	String XM_SOLON = "xm-solon";

	String XM_TEST = "xm-test";

	String XM_ZIP = "xm-zip";

	String XM_HEALTH = "xm-health";

	String XM_STOP = "xm-stop";

	String XM_EXCEPTION_RECORD = "xm-exception-record";

	String XM_EXCEPTION_HANDLER = "xm-exception-handler";

	String XM_EXCEPTION = "xm-exception";

	String XM_CORE_WEB = "xm-core-web";

}
