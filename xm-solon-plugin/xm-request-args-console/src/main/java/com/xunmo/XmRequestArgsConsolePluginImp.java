		...
		if (isEnabled) {
			ThreadUtil.execute(() -> {
				if (StrUtil.equalsIgnoreCase(type, defaultType)) {
					HandlerExtUtil.toBuildExtRequestHandler(context, defaultIndex, ArgsConsoleHandler.class,
							ArgsConsoleHandlerDefault.class);
				}
				else {
					HandlerExtUtil.toBuildExtRequestFilter(context, defaultIndex, ArgsConsoleFilter.class,
							ArgsConsoleFilterDefault.class);
				}
			});
		}
