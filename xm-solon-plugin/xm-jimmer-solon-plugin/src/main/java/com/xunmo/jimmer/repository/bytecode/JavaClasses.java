...
public class JavaClasses {
	...
	public static Class<?> define(byte[] bytecode, Class<?> repositoryInterface) {
		try {
			return LOOKUP_DEFINE_CLASS != null ? defineForJava9(bytecode, repositoryInterface)
					: defineForJava8(bytecode, repositoryInterface);
		}
		catch (IllegalAccessException ex) {
			throw new IllegalArgumentException("Cannot create implementation class for \"" + repositoryInterface + "\"",
					ex);
		}
	...
	private static Class<?> defineForJava9(byte[] bytecode, Class<?> repositoryInterface)
	...
	private static Class<?> defineForJava8(byte[] bytecode, Class<?> repositoryInterface)
			throws InvocationTargetException, IllegalAccessException {
		return (Class<?>) CLASS_LOADER_DEFINE_CLASS.invoke(repositoryInterface.getClassLoader(),
				ClassCodeWriter.implementationClassName(repositoryInterface), bytecode, 0, bytecode.length);
	}
