...
public abstract class MethodCodeWriter implements Constants {
	...
	public void write() {
		...
		QueryMethod queryMethod = QueryMethod.of(parent.ctx, ImmutableType.get(parent.getDomainType()), method);
		...
		mv = parent.getClassVisitor()
			.visitMethod(Opcodes.ACC_PUBLIC, method.getName(), Type.getMethodDescriptor(method), null, null);
		mv.visitCode();
	...
	private void writeCode(QueryMethod queryMethod, List<Integer> slots) {
		...
		mv.visitFieldInsn(Opcodes.GETSTATIC, parent.getImplInternalName(), queryMethodFieldName(),
		...
		mv.visitMethodInsn(Opcodes.INVOKESTATIC, QUERY_EXECUTORS_INTERNAL_NAME, "execute",
				QUERY_EXECUTORS_METHOD_DESCRIPTOR, false);
