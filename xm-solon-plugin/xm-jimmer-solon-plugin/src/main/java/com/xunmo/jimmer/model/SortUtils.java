...
public class SortUtils {
	...
	public static Sort toSort(String... codes) {
		return Sort.by(Order.makeCustomOrders((path, orderMode, nullOrderMode) -> {
			...
			return new Sort.Order(orderMode == OrderMode.DESC ? Sort.Direction.DESC : Sort.Direction.ASC, path,
					nullHandling);
		}, codes));
	}
	...
	public static Sort toSort(TypedProp.Scalar<?, ?>... props) {
		...
		for (TypedProp.Scalar<?, ?> prop : props) {
			...
			Sort.Order order = new Sort.Order(prop.isDesc() ? Sort.Direction.DESC : Sort.Direction.ASC, ip.getName(),
					prop.isNullsFirst() ? Sort.NullHandling.NULLS_FIRST
							: prop.isNullsLast() ? Sort.NullHandling.NULLS_LAST : Sort.NullHandling.NATIVE);
			orders.add(order);
		}
