...
class QueryMethodParser {
	...
	private QueryMethodParser(Context ctx, ImmutableType type, Method method) {
		...
		if (pageableParamIndex != -1 && sortParamIndex != -1) {
			throw new IllegalArgumentException("Cannot have parameters of type \"" + Pageable.class.getName()
					+ "\" and \"" + Sort.class.getName() + "\" at the same time");
		}
	}
	...
	private QueryMethod parse() {
		try {
			...
			throw new IllegalArgumentException(
					"Illegal abstract spring-data method \"" + method + "\": " + ex.getMessage(), ex);
		}
	}
	...
	private QueryMethod parse0() {
		...
		if (method.getReturnType() == Page.class && pageableParamIndex == -1) {
			throw new IllegalArgumentException(
					"Return type \"" + Page.class + "\" requires parameter whose type is \"" + Pageable.class + "\"");
		}
		if (method.getReturnType() != Page.class && pageableParamIndex != -1) {
			throw new IllegalArgumentException("The parameter whose type is \"" + Pageable.class
					+ "\" requires the return type \"" + Page.class + "\"");
		}
		if (query.getAction() == Query.Action.FIND) {
			...
			if (returnType == List.class || returnType == Collection.class || returnType == Iterable.class
					|| returnType == Page.class || returnType == Optional.class) {
				Type genericReturnType = method.getGenericReturnType();
				if (!(genericReturnType instanceof ParameterizedType)) {
					throw new IllegalArgumentException("Return type must be parameterized type when raw return type is "
							+ "\"" + List.class.getName() + "\", \"" + Collection.class.getName() + "\", \""
							+ Iterator.class.getName() + "\", \"" + Page.class.getName() + "\" or \"" + Optional.class
							+ "\"");
				}
			}
			if (query.getSelectedPath() != null) {
				if (actualElementType != query.getSelectedPath().getType()) {
					throw new IllegalArgumentException(
							"The returned element type must be \"" + query.getSelectedPath().getType() + "\"");
				}
			}
		}
		...
		if (fetcherParamIndex != -1) {
			...
			if (query.getSelectedPath() != null) {
				throw new IllegalArgumentException(
						"Cannot explicitly select columns when there is a fetcher parameter");
			}
			...
			if (parameterizedType.getActualTypeArguments()[0] != type.getJavaClass()) {
				throw new IllegalArgumentException(
						"The type argument of fetch parameter must be \"" + type.getJavaClass() + "\"");
			}
		}
		...
		return new QueryMethod(method, query, pageableParamIndex, sortParamIndex, fetcherParamIndex);
	}

	private Predicate resolve(Predicate predicate) {
		if (predicate instanceof AndPredicate) {
			return AndPredicate.of(((AndPredicate) predicate).getPredicates()
				.stream()
				.map(this::resolve)
			...
			return OrPredicate
				.of(((OrPredicate) predicate).getPredicates().stream().map(this::resolve).collect(Collectors.toList()));
		}
	...
	private Predicate resolve(PropPredicate propPredicate) {
		if (!propPredicate.getPath().isScalar() && propPredicate.getOp() != PropPredicate.Op.NULL
				&& propPredicate.getOp() != PropPredicate.Op.NOT_NULL) {
			throw new IllegalArgumentException("Illegal property \"" + propPredicate.getPath() + "\" of \""
		...
		switch (propPredicate.getOp()) {
			...
			case FALSE:
				if (!Classes.matches(propPredicate.getPath().getType(), boolean.class)) {
					throw new IllegalArgumentException("Illegal property \"" + propPredicate.getPath()
	...
	private Param nextParam(PropPredicate predicate) {
		...
		if (paramIndex >= parameterTypes.length) {
			throw new IllegalArgumentException("No enough parameters for the property \"" + predicate.getPath()
					+ "\" of \"" + predicate.getPath().getSource() + "\"");
		}
		...
		if (predicate.getOp() == PropPredicate.Op.IN || predicate.getOp() == PropPredicate.Op.NOT_IN) {
			boolean valid = false;
			ParameterizedType parameterizedType = genericParameterTypes[paramIndex] instanceof ParameterizedType
					? (ParameterizedType) genericParameterTypes[paramIndex] : null;
			if (parameterizedType != null) {
				if (parameterizedType.getRawType() == Collection.class
						|| parameterizedType.getRawType() == List.class) {
					valid = Classes
						.boxTypeOf(predicate.getPath().getType()) == parameterizedType.getActualTypeArguments()[0];
				}
			}
			if (!valid) {
				expectedTypeName = "Collection<" + Classes.boxTypeOf(predicate.getPath().getType()).getName() + '>';
				actualTypeName = parameterizedType == null ? parameterTypes[paramIndex].getName()
		...
		if (expectedTypeName != null) {
			throw new IllegalArgumentException(
					"This type of " + (isCollection ? "the collection whose element is the " : "") + "property \""
							+ predicate.getPath() + "\" is \"" + expectedTypeName + "\", but the type of parameters["
							+ paramIndex + "] of java method is \"" + actualTypeName + "\"");
		}
	...
	private static boolean isImplicitParameterType(Class<?> type) {
		return Pageable.class.isAssignableFrom(type) || Sort.class.isAssignableFrom(type)
				|| Fetcher.class.isAssignableFrom(type) || Class.class.isAssignableFrom(type)
	...
	private int implicitParameterIndex(Class<?> type) {
		...
		for (int i = 0; i < parameterTypes.length; i++) {
			if (type.isAssignableFrom(parameterTypes[i])) {
				if (index != -1) {
					throw new IllegalArgumentException("Both parameters[" + index + "] and parameters[" + i
							+ "] are of type \"" + type.getName() + "\"");
				}
				index = i;
			}
		}
	...
	static class Param {
		...
		public String toString() {
			return "Param{" + "index=" + index + ", logicIndex=" + logicIndex + '}';
		}
