...
class PathParser {
	...
	public Path parse(Source source, ImmutableType type) {
		if (!parse0(source, type)) {
			throw new IllegalArgumentException(
					"Cannot resolve the property name \"" + source + "\" by \"" + type + "\"");
		}
	...
	private boolean parse0(Source source, ImmutableProp prop) {
		...
		if (!restSource.isEmpty() && (prop.getTargetType() == null)) {
			throw new IllegalArgumentException(
					"Cannot resolve the property name \"" + source + "\" by \"" + prop.getDeclaringType() + "\"");
		}
	...
	private static Source trimByProp(Source source, ImmutableProp prop) {
		...
		for (int i = 0; i < len; i++) {
			...
			boolean matched = toLowerCase ? Character.toLowerCase(ch) == expectedCh : ch == expectedCh;
			if (!matched) {
				return null;
			}
