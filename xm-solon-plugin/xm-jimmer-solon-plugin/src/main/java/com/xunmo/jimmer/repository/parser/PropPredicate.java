...
public abstract class PropPredicate implements Predicate {
	...
	public static PropPredicate of(Context ctx, boolean allowCollection, boolean allIgnoreCase, Source source,
			ImmutableType type) {
	...
	public enum Op {
		...
		TRUE("IsTrue"), FALSE("IsFalse"), EQ("Equals"), NE("NotEqual"), LT("LessThan"), LE("LessThanEqual"),
		GT("GreaterThan"), GE("GreaterThanEqual"), NULL("IsNull"), NOT_NULL("IsNot"), IN("In"), NOT_IN("NotIn"),
		BETWEEN("Between"), NOT_BETWEEN("NotBetween"), LIKE("Like"), NOT_LIKE("NotLike");
	...
	static class Unresolved extends PropPredicate {
		...
		Resolved resolve() {
			return new Resolved(path, op, insensitive, likeMode, -1, -1, -1, -1);
		}

		Resolved resolve(QueryMethodParser.Param param) {
			return new Resolved(path, op, insensitive, likeMode, param.getIndex(), param.getLogicIndex(), -1, -1);
		}

		Resolved resolve(QueryMethodParser.Param param, QueryMethodParser.Param param2) {
			return new Resolved(path, op, insensitive, likeMode, param.getIndex(), param.getLogicIndex(),
					param2.getIndex(), param2.getLogicIndex());
		}

		@Override
		public String toString() {
			return "UnresolvedPredicate{" + "path=" + path + ", op=" + op + ", insensitive=" + insensitive
					+ ", likeMode=" + likeMode + '}';
		}
	...
	static class Resolved extends PropPredicate {
		...
		public Resolved(Path path, Op op, boolean insensitive, LikeMode likeMode, int paramIndex, int logicParamIndex,
				int paramIndex2, int logicParamIndex2) {
			super(path, op, insensitive, likeMode);
		...
		@Override
		public String toString() {
			return "ResolvedPredicate{" + "path=" + path + ", op=" + op + ", insensitive=" + insensitive + ", likeMode="
					+ likeMode + ", paramIndex=" + paramIndex + ", logicParamIndex=" + logicParamIndex
					+ ", paramIndex2=" + paramIndex2 + ", logicParamIndex2=" + logicParamIndex2 + '}';
		}
