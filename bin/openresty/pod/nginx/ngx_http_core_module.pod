...
the
C<If-Modified-Since> request header field is ignored (0.7.34);
...
Rate limit can also be set in the C<$limit_rate> variable.
...
The C<spdy> parameter (1.3.15-1.9.4) allows accepting
L<SPDY|ngx_http_spdy_module> connections on this port.
Normally, for this to work the C<ssl> parameter should be
