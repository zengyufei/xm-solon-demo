		...
		if (isEnabled) {
			ThreadUtil.execute(() -> {
				if (StrUtil.equalsIgnoreCase(type, defaultType)) {
					HandlerExtUtil.toBuildExtRequestHandler(context, defaultIndex, RequestXssHandler.class,
							RequestXssHandlerDefault.class);
				}
				else {
					HandlerExtUtil.toBuildExtRequestFilter(context, defaultIndex, RequestXssFilter.class,
							RequestXssFilterDefault.class);
				}
			});
		}
