...
public class QueryExecutors {
	...
	public static Object execute(JSqlClientImplementor sqlClient, ImmutableType type, QueryMethod queryMethod,
			Pageable pageable, Sort sort, Fetcher<?> fetcher, Object[] args) {
		...
		if (queryData.getAction() == Query.Action.DELETE) {
			...
			ConfigurableRootQuery<?, Object> query = Queries.createQuery(sqlClient, type, ExecutionPurpose.QUERY, false,
					(q, table) -> {
						q.where(astPredicate(table, queryData.getPredicate(), args));
						for (Query.Order order : queryData.getOrders()) {
							q.orderBy(order.getOrderMode() == OrderMode.DESC
						...
						if (queryData.getSelectedPath() != null) {
							return q
								.select((Expression<Object>) astSelection(table, queryData.getSelectedPath(), false));
						}
						...
						if (queryData.getAction() == Query.Action.EXISTS) {
							return q
								.select(table.<Expression<Object>>get(table.getImmutableType().getIdProp().getName()));
						}
				...
				case FIND:
					...
					if (returnType == Page.class) {
						if (pageable != null) {
							...
							List<Object> entities = query.limit(pageable.getPageSize(), (int) pageable.getOffset())
								.execute();
	...
	private static org.babyfish.jimmer.sql.ast.Predicate astPredicate(Table<?> table, Predicate predicate,
			Object[] args) {
		if (predicate == null) {
			return null;
		}
		if (predicate instanceof PropPredicate) {
			...
			switch (propPredicate.getOp()) {
				...
				case NULL:
					return astSelection instanceof Expression<?> ? ((Expression<?>) astSelection).isNull()
							: ((Table<?>) astSelection).isNull();
				case NOT_NULL:
					return astSelection instanceof Expression<?> ? ((Expression<?>) astSelection).isNotNull()
				...
				case BETWEEN: {
					...
					Comparable min = (Comparable) insensitive(propPredicate.isInsensitive(),
							args[propPredicate.getLogicParamIndex()]);
					Comparable max = (Comparable) insensitive(propPredicate.isInsensitive(),
				...
				case NOT_LIKE: {
					String pattern = (String) args[propPredicate.getLogicParamIndex()];
					return pattern == null || pattern.isEmpty() ? null : propPredicate.isInsensitive()
				...
				case LT: {
					astSelection = insensitive(propPredicate.isInsensitive(), astSelection);
					Comparable value = (Comparable) insensitive(propPredicate.isInsensitive(),
							args[propPredicate.getLogicParamIndex()]);
		...
		if (predicate instanceof AndPredicate) {
			...
			org.babyfish.jimmer.sql.ast.Predicate[] subAstPredicates = new org.babyfish.jimmer.sql.ast.Predicate[subPredicates
				.size()];
			int index = 0;
