...
public class JRepositoryImpl<E, ID> implements JRepository<E, ID> {
	...
	public JRepositoryImpl(JSqlClient sqlClient, Class<E> entityType) {
		...
		if (!immutableType.isEntity()) {
			throw new IllegalArgumentException(
					"\"" + entityType + "\" is not entity type decorated by @" + Entity.class.getName());
		}
	}
	...
	public List<E> findAll(Fetcher<E> fetcher, Sort sort) {
		MutableRootQueryImpl<Table<E>> query = new MutableRootQueryImpl<>(sqlClient, immutableType,
				ExecutionPurpose.QUERY, false);
	...
	public Page<E> findAll(Pageable pageable, Fetcher<E> fetcher) {
		...
		return pager(pageable).execute(query.select(table.fetch(fetcher)));
	}
	...
	public <S extends E> SimpleSaveResult<S> save(@NotNull S entity, SaveMode mode) {
		return sqlClient.getEntities().saveCommand(entity).setMode(mode).execute();
	}
	...
	public <S extends E> BatchSaveResult<S> saveAll(@NotNull Iterable<S> entities, SaveMode mode) {
		return sqlClient.getEntities().batchSaveCommand(Utils.toCollection(entities)).setMode(mode).execute();
	}

	@NotNull
	@Override
	public <S extends E> BatchEntitySaveCommand<S> saveAllCommand(@NotNull Iterable<S> entities) {
		return sqlClient.getEntities().batchSaveCommand(Utils.toCollection(entities));
	}

	@Override
	public int delete(@NotNull E entity, DeleteMode mode) {
		return sqlClient.getEntities()
			.delete(entityType, ImmutableObjects.get(entity, immutableType.getIdProp().getId()), mode)
	...
	public int deleteAll(@NotNull Iterable<? extends E> entities, DeleteMode mode) {
		return sqlClient.getEntities()
			.batchDelete(entityType,
					Utils.toCollection(entities)
						.stream()
	...
	public void deleteAll() {
		Mutations.createDelete(sqlClient, immutableType, (d, t) -> {
		}).execute();
	}
	...
	private ConfigurableRootQuery<?, E> createQuery(Fetcher<E> fetcher, TypedProp.Scalar<?, ?>[] sortedProps) {
		...
		for (TypedProp.Scalar<?, ?> sortedProp : sortedProps) {
			if (!sortedProp.unwrap().getDeclaringType().isAssignableFrom(immutableType)) {
				throw new IllegalArgumentException("The sorted field \"" + sortedProp
						+ "\" does not belong to the type \"" + immutableType + "\" or its super types");
			}
	...
	private static class PagerImpl implements Pager {
		...
		public <T> Page<T> execute(ConfigurableRootQuery<?, T> query) {
			return PagingQueries.execute(query, pageIndex, pageSize, (entities, totalCount,
					queryImplementor) -> new PageImpl<>(entities, PageRequest.of(pageIndex, pageSize, Utils
