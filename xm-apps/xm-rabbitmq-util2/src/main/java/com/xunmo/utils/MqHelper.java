...
public class MqHelper {
	...
	public static synchronized void initSolon() {
		...
		synchronized (initLock) {
			...
			if (app != null) {
				...
				final MqConnectionPoolObjectFactory mqConnectionPoolObjectFactory = new MqConnectionPoolObjectFactory(
						mqPoolConfig);
	 ...
	 * @param connection 连接
	 */
	private static void sendMsg(Connection connection, MqConfig mqConfig, String msg, Integer expiration,
			java.util.function.BiConsumer<Channel, SendAction> sendFunc) throws IOException {
		...
		try {
			...
			if (isDelay) {
				...
				tryDo(() -> channel.basicPublish(changeName, finalRoutingKey, props.build(),
						msg.getBytes(StandardCharsets.UTF_8)), () -> sendErrorMsg);
			}
			else {
				if (StrUtil.isNotBlank(changeName)) {
					...
					props.priority(null);
					tryDo(() -> channel.basicPublish(finalRoutingKey, queueName, props.build(),
							msg.getBytes(StandardCharsets.UTF_8)), () -> sendErrorMsg);
				}
			}
		}
	 ...
	 */
	private static void consumeMsg(Connection connection, MqConfig mqConfig,
	...
	private static Channel initConsumeChannel(Connection connection, MqConfig mqConfig) throws IOException {
		...
		if (deadConfig != null) {
			...
			if (!StrUtil.isBlankOrUndefined(deadChangeName)) {
				...
				tryDo(() -> channel.queueBind(deadQueueName, deadChangeName, finalDeadRoutingKey),
						() -> "绑定死信交换机和死信队列失败");
			}
		}
