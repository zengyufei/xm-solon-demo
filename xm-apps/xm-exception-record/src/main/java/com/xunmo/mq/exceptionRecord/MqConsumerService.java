                ...
                MqHelper.consumeMsg(
                        mqConfig, (channel, content) -> {
                            ...
                            log.info("{} 消息队列-收到-队列: {} {} {}", MqSendService.BUSINESS_NAME,
                                    exceptionRecordInput.getMethod(),
                                    exceptionRecordInput.getUri(),
                                    exceptionRecordInput.getParams());
                            try {
                                final SimpleSaveResult<ExceptionRecord> saveResult = sqlClient.save(exceptionRecordInput, SaveMode.INSERT_ONLY);
