...
public class LamUtil {
	...
	public static String join(List<String> originList, String delimiter) {
		...
		return originList.stream().filter(StrUtil::isNotBlank).collect(Collectors.joining(delimiter));
	}
	 ...
	 */
	@SafeVarargs
	public static <T, R> List<R> filtersMapToList(List<T> originList, Function<T, R> function,
			Predicate<T>... filters) {
		if (CollUtil.isEmpty(originList)) {
			return new ArrayList<>();
		}
	...
	/**
	 * 根据传入的 Predicate 列表过滤 List 中的元素，然后再将剩余的元素转换成一个新的元素，最后返回去重后的
	 * List。这个方法遵循空值不处理和去重后顺序不变的原则。
	 * @param originList 要处理的 List，该方法会删除其中的 null 值
	 * @param function 转换函数，用于将剩余的元素转换成一个新的元素。
	 * @param filters 要用于过滤的 Predicate 列表
	 ...
	 */
	@SafeVarargs
	public static <T, R> List<R> filtersDistinctMapToList(List<T> originList, Function<T, R> function,
			Predicate<T>... filters) {
		if (CollUtil.isEmpty(originList)) {
			return new ArrayList<>();
		}
	...
	/**
	 * 根据传入的 Predicate 列表过滤 List 中的元素，然后再将剩余的元素转换成一个新的元素，最后返回先转换后去重的
	 * List。这个方法遵循空值不处理和去重后顺序不变的原则。
	 * @param originList 要处理的 List，该方法会删除其中的 null 值
	 * @param function 转换函数，用于将剩余的元素转换成一个新的元素。
	 * @param filters 要用于过滤的 Predicate 列表
	 ...
	 */
	@SafeVarargs
	public static <T, R> List<R> filtersMapDistinctToList(List<T> originList, Function<T, R> function,
			Predicate<T>... filters) {
		if (CollUtil.isEmpty(originList)) {
			return new ArrayList<>();
		}
	 ...
	 */
	@SafeVarargs
	public static <R> List<R> filterBlankDistinctMapToList(List<String> originList, Function<String, R> function,
	 ...
	 */
	@SafeVarargs
	public static <T> List<String> mapFiltersBlankDistinctToList(List<T> originList, Function<T, String> function,
			Predicate<String>... filters) {
		if (CollUtil.isEmpty(originList)) {
			return new ArrayList<>();
		}
	 ...
	 */
	public static <T, R> boolean mapDistinctAnyMatch(List<T> originList, Function<T, R> mapper,
	...
	public static <T, R> List<R> mapToList(List<T> originList, Function<T, R>... filters) {
		...
		return originList.stream()
			.flatMap(t -> Arrays.stream(filters).filter(Objects::nonNull).map(f -> f.apply(t)).filter(Objects::nonNull))
			.collect(Collectors.toList());
	}
	 ...
	 */
	@SuppressWarnings({ "all" })
	public static <T, R> List<R> mapDistinctFiltersToList(List<T> originList, Function<T, R> mapper,
			Predicate<R>... filters) {
		if (CollUtil.isEmpty(originList)) {
			return new ArrayList<>();
		}
	 ...
	 * @return Map结果
	 */
	public static <K, V> Map<K, V> filterToBeanMap(List<V> originList, Predicate<V> filter,
	 ...
	 * @return Map结果
	 */
	public static <K, V> Map<K, V> filterToBeanMergeMap(List<V> originList, Predicate<V> filter,
	 ...
	 */
	@SafeVarargs
	public static <K, V> Map<K, V> filtersToBeanMap(List<V> originList, Function<V, K> keyExtractor,
			Predicate<V>... filters) {
		if (CollUtil.isEmpty(originList)) {
			return new HashMap<>();
		}
	 ...
	 * @return Map结果
	 */
	public static <K, V, S> Map<K, S> toMap(List<V> originList, Function<V, K> keyExtractor,
	 ...
	 * @return Map结果
	 */
	public static <K, V, S> Map<K, S> toLinkedMap(List<V> originList, Function<V, K> keyExtractor,
			Function<V, S> valueExtractor) {
		if (CollUtil.isEmpty(originList)) {
			return new HashMap<>();
		}
	 ...
	 * @return Map结果
	 */
	public static <K, V, S> Map<K, S> toMergeMap(List<V> originList, Function<V, K> keyExtractor,
	 ...
	 * @return Map结果
	 */
	public static <K, V, S> Map<K, S> toMergeLinkedMap(List<V> originList, Function<V, K> keyExtractor,
			Function<V, S> valueExtractor, BinaryOperator<S> mergeExtractor) {
		if (CollUtil.isEmpty(originList)) {
			return new HashMap<>();
		}
	 ...
	 * @return Map结果
	 */
	public static <K, V> Map<K, V> toBeanMergeMap(List<V> originList, Function<V, K> keyExtractor,
	 ...
	 * @return Map结果
	 */
	public static <K, V> Map<K, V> toBeanMergeLinkedMap(List<V> originList, Function<V, K> keyExtractor,
			BinaryOperator<V> mergeExtractor) {
		if (CollUtil.isEmpty(originList)) {
			return new HashMap<>();
		}
	 ...
	 */
	public static <K, V, S> Map<K, S> filterToMap(List<V> originList, Predicate<V> filter, Function<V, K> keyExtractor,
			Function<V, S> valueExtractor) {
		if (CollUtil.isEmpty(originList)) {
			return new HashMap<>();
		}
	 ...
	 * @return 经过过滤、转化、聚合操作的 Map
	 */
	public static <K, V, S> Map<K, S> filterToMergeMap(List<V> originList, Predicate<V> filter,
	 ...
	 * @return 经过过滤、转化、聚合操作的 Map
	 */
	public static <K, V, S> Map<K, S> filterToMergeLinkedMap(List<V> originList, Predicate<V> filter,
			Function<V, K> keyExtractor, Function<V, S> valueExtractor, BinaryOperator<S> mergeExtractor) {
		if (CollUtil.isEmpty(originList)) {
			return new HashMap<>();
		}
	 ...
	 */
	@SafeVarargs
	public static <K, V, S> Map<K, S> filtersToMergeMap(List<V> originList, Function<V, K> keyExtractor,
	 ...
	 */
	@SafeVarargs
	public static <K, V, S> Map<K, S> filtersToMergeLinkedMap(List<V> originList, Function<V, K> keyExtractor,
			Function<V, S> valueExtractor, BinaryOperator<S> mergeExtractor, Predicate<V>... filters) {
		if (CollUtil.isEmpty(originList)) {
			return new HashMap<>();
		}
	...
	/**
	 * 根据某一条目必要属性值，对待处理列表中的数据进行分组操作，返回一个 Map，Map 的键为对应的属性字段，Map
	 * 的值为具有对应属性值的另一个属性列表经过转化逻辑之后得到的数据列表。
	 * @param <K> 返回的 Map 中键的类型
	 * @param <V> 待处理列表中元素的类型
