...
public class HandlerExtUtil {
	...
	public static <T extends XmHandlerExt, S extends XmHandlerExt> void toBuildExtRequestHandler(AopContext context,
			int defaultIndex, Class<T> superClass, Class<S> defaultClass) {
	...
	public static <T extends XmFilterExt, S> void toBuildExtRequestFilter(AopContext context, int defaultIndex,
			Class<T> superClass, Class<S> defaultClass) {
		final SolonApp app = Solon.app();
		AtomicInteger index = new AtomicInteger(defaultIndex);
		// 向外提供钩子
		context.beanOnloaded(aopContext -> {
