...
public class XmAutoTranPluginImp implements Plugin {
    ...
    public void start(AopContext context) {
        ...
        props.loadAddIfAbsent(XmPackageConstants.XM_AUTO_TRAN + ".yml");
//        final SolonApp app = Solon.app();

        // 自动给符合的方法名自动进入事务;
        if (props.getBool(XmPluginConfig.xmTranEnable, false)) {
...
//        Solon.context().beanAroundAdd(TranAnno.class, new TranAfterInterceptor());
        ...
        if (XmPackageConstants.IS_CONSOLE_LOG) {
            log.info("{} 包加载完毕!", XmPackageConstants.XM_AUTO_TRAN);
        }else {
            System.out.println(XmPackageConstants.XM_AUTO_TRAN + " 包加载完毕!");
        }
    }
    ...
    public void stop() throws Throwable {
        if (XmPackageConstants.IS_CONSOLE_LOG) {
            log.info("{} 插件关闭!", XmPackageConstants.XM_AUTO_TRAN);
        }else {
            System.out.println(XmPackageConstants.XM_AUTO_TRAN + " 插件关闭!");
        }
    }
}
