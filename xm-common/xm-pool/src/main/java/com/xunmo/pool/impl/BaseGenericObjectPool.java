...
import com.xunmo.pool.BaseObject;
import com.xunmo.pool.PooledObject;
import com.xunmo.pool.PooledObjectState;
import com.xunmo.pool.SwallowedExceptionListener;
...
/**
 * Base class that provides common functionality for {@link GenericObjectPool} and
 * {@link GenericKeyedObjectPool}. The primary reason this class exists is reduce code
 * duplication between the two pool implementations.
 *
...
public abstract class BaseGenericObjectPool<T, E extends Exception> extends BaseObject implements AutoCloseable {
	...
	/**
	 * The size of the caches used to store historical data for some attributes so that
	 * rolling means may be calculated.
	 */
	...
	/**
	 * Class loader for evictor thread to use since, in a JavaEE or similar environment,
	 * the context class loader for the evictor thread may not have visibility of the
	 * correct factory. See POOL-161. Uses a weak reference to avoid potential memory
	 * leaks if the Pool is discarded rather than closed.
	 */
	 ...
	 * @param jmxNameBase The default base JMX name for the new pool unless overridden by
	 * the config
	 * @param jmxNamePrefix Prefix to be used for JMX name for the new pool
	 */
	public BaseGenericObjectPool(final BaseObjectPoolConfig<T> config, final String jmxNameBase,
			final String jmxNamePrefix) {
	 ...
	 * <p>
	 * Statistics may not accurately reflect snapshot state at the time of the exception
	 * because we do not want to lock the pool when gathering this information.
	 * </p>
	...
	/**
	 * Closes the pool, destroys the remaining idle objects and, if registered in JMX,
	 * deregisters it.
	 */
	@Override
	...
	/**
	 * Tries to ensure that the configured minimum number of idle instances are available
	 * in the pool.
	...
	/**
	 * Perform {@code numTests} idle object eviction tests, evicting examined objects that
	 * meet the criteria for eviction. If {@code testWhileIdle} is true, examined objects
	 * are validated when visited (and removed if invalid); otherwise only objects that
	 * have been idle for more than {@code minEvicableIdleTimeMillis} are removed.
	 * @throws E when there is a problem evicting idle objects.
	 */
	public abstract void evict() throws E;

	/**
	 * Gets whether to block when the {@code borrowObject()} method is invoked when the
	 * pool is exhausted (the maximum number of "active" objects has been reached).
	 * @return {@code true} if {@code borrowObject()} should block when the pool is
	 * exhausted
	...
	/**
	 * Sets whether to block when the {@code borrowObject()} method is invoked when the
	 * pool is exhausted (the maximum number of "active" objects has been reached).
	 * @param blockWhenExhausted {@code true} if {@code borrowObject()} should block when
	...
	/**
	 * Gets the total number of objects created for this pool over the lifetime of the
	 * pool.
	...
	/**
	 * Gets the stack trace for the call that created this pool. JMX registration may
	 * trigger a memory leak so it is important that pools are deregistered when no longer
	 * used by calling the {@link #close()} method. This method is provided to assist with
	 * identifying code that creates but does not close it thereby creating a memory leak.
	 ...
	 * validation during {@code borrowObject()} over the lifetime of the pool.
	...
	/**
	 * Gets the total number of objects destroyed by the evictor associated with this pool
	...
	/**
	 * Gets the total number of objects destroyed by this pool over the lifetime of the
	 * pool.
	...
	/**
	 * Gets the duration to sleep between runs of the idle object evictor thread. When
	...
	/**
	 * Gets the name of the {@link EvictionPolicy} implementation that is used by this
	 * pool.
	...
	/**
	 * Sets the name of the {@link EvictionPolicy} implementation that is used by this
	 * pool. The Pool will attempt to load the class using the thread context class
	 * loader. If that fails, the use the class loader for the {@link EvictionPolicy}
	 * interface.
	 * @param evictionPolicyClassName the fully qualified class name of the new eviction
	 * policy
	 * @see #getEvictionPolicyClassName()
	 * @since 2.6.0 If loading the class using the thread context class loader fails, use
	 * the class loader for the {@link EvictionPolicy} interface.
	 */
	...
	/**
	 * Gets the timeout that will be used when waiting for the Evictor to shutdown if this
	 * pool is closed and it is the only pool still using the the value for the Evictor.
	...
	/**
	 * Sets the timeout that will be used when waiting for the Evictor to shutdown if this
	 * pool is closed and it is the only pool still using the value for the Evictor.
	 * @param evictorShutdownTimeout the timeout in milliseconds that will be used while
	 * waiting for the Evictor to shut down.
	 * @since 2.10.0
	 */
	public final void setEvictorShutdownTimeout(final Duration evictorShutdownTimeout) {
		this.evictorShutdownTimeoutDuration = PoolImplUtils.nonNull(evictorShutdownTimeout,
	 ...
	 * Gets the timeout that will be used when waiting for the Evictor to shutdown if this
	 * pool is closed and it is the only pool still using the the value for the Evictor.
	 * @return The timeout in milliseconds that will be used while waiting for the Evictor
	 * to shut down.
	 * @deprecated Use {@link #getEvictorShutdownTimeoutDuration()}.
	 */
	@Deprecated
	 ...
	 * Sets the timeout that will be used when waiting for the Evictor to shutdown if this
	 * pool is closed and it is the only pool still using the value for the Evictor.
	 * @param evictorShutdownTimeoutMillis the timeout in milliseconds that will be used
	 * while waiting for the Evictor to shut down.
	...
	/**
	 * Gets whether or not the pool serves threads waiting to borrow objects fairly. True
	 * means that waiting threads are served as if waiting in a FIFO queue.
	 * @return {@code true} if waiting threads are to be served by the pool in arrival
	 * order
	 */
	...
	/**
	 * Gets the name under which the pool has been registered with the platform MBean
	 * server or {@code null} if the pool has not been registered.
	 * @return the JMX name
	 */
	...
	/**
	 * Gets whether the pool has LIFO (last in, first out) behavior with respect to idle
	 * objects - always returning the most recently used object from the pool, or as a
	 * FIFO (first in, first out) queue, where the pool always returns the oldest object
	 * in the idle object pool.
	 * @return {@code true} if the pool is configured with LIFO behavior or {@code false}
	 * if the pool is configured with FIFO behavior
	 * @see #setLifo
	 */
	...
	/**
	 * Sets whether the pool has LIFO (last in, first out) behavior with respect to idle
	 * objects - always returning the most recently used object from the pool, or as a
	 * FIFO (first in, first out) queue, where the pool always returns the oldest object
	 * in the idle object pool.
	 * @param lifo {@code true} if the pool is to be configured with LIFO behavior or
	 * {@code false} if the pool is to be configured with FIFO behavior
	 * @see #getLifo()
	 */
	 ...
	 * @return {@code true} if abandoned object removal is configured for this pool and
	 * removal events are to be logged otherwise {@code false}
	...
	/**
	 * Gets the maximum number of objects that can be allocated by the pool (checked out
	 * to clients, or idle awaiting checkout) at a given time. When negative, there is no
	 ...
	 * for no limit.
	 * @param maxTotal The cap on the total number of object instances managed by the
	 * pool. Negative values mean that there is no limit to the number of objects
	...
	/**
	 * Gets the maximum duration the {@code borrowObject()} method should block before
	 * throwing an exception when the pool is exhausted and {@link #getBlockWhenExhausted}
	 * is true. When less than 0, the {@code borrowObject()} method may block
	 * indefinitely.
	...
	/**
	 * Gets the maximum amount of time (in milliseconds) the {@code borrowObject()} method
	...
	/**
	 * Sets the maximum amount of time (in milliseconds) the {@code borrowObject()} method
	 * should block before throwing an exception when the pool is exhausted and
	 * {@link #getBlockWhenExhausted} is true. When less than 0, the
	 * {@code borrowObject()} method may block indefinitely.
	 * @param maxWaitMillis the maximum number of milliseconds {@code borrowObject()} will
	...
	/**
	 * Gets the mean time objects are active for based on the last
	 * {@link #MEAN_TIMING_STATS_CACHE_SIZE} objects returned to the pool.
	 * @return mean time an object has been checked out from the pool among recently
	 * returned objects
	 */
	 ...
	 * {@link #MEAN_TIMING_STATS_CACHE_SIZE} objects borrowed from the pool.
	 * @return mean time in milliseconds that a recently served thread has had to wait to
	...
	/**
	 * Gets the mean time objects are idle for based on the last
	 * {@link #MEAN_TIMING_STATS_CACHE_SIZE} objects borrowed from the pool.
	 * @return mean time an object has been idle in the pool among recently borrowed
	 * objects
	 */
	...
	/**
	 * Gets the minimum amount of time an object may sit idle in the pool before it is
	 * eligible for eviction by the idle object evictor (if any - see
	 * {@link #setTimeBetweenEvictionRuns(Duration)}). When non-positive, no objects will
	 * be evicted from the pool due to idle time alone.
	 * @return minimum amount of time an object may sit idle in the pool before it is
	 ...
	 * {@link #setTimeBetweenEvictionRuns(Duration)}). When non-positive, no objects will
	 * be evicted from the pool due to idle time alone.
	 * @param minEvictableIdleTime minimum amount of time an object may sit idle in the
	...
	public final void setMinEvictableIdleTime(final Duration minEvictableIdleTime) {
		this.minEvictableIdleDuration = PoolImplUtils.nonNull(minEvictableIdleTime,
	 ...
	 * Gets the minimum amount of time an object may sit idle in the pool before it is
	 * eligible for eviction by the idle object evictor (if any - see
	 * {@link #setTimeBetweenEvictionRunsMillis(long)}). When non-positive, no objects
	 * will be evicted from the pool due to idle time alone.
	 * @return minimum amount of time an object may sit idle in the pool before it is
	 * eligible for eviction
	 * @see #setMinEvictableIdleTimeMillis
	 * @see #setTimeBetweenEvictionRunsMillis
	...
	/**
	 * Gets the maximum number of objects to examine during each run (if any) of the idle
	 * object evictor thread. When positive, the number of tests performed for a run will
	 * be the minimum of the configured value and the number of idle instances in the
	 * pool. When negative, the number of tests performed will be
	 * <code>ceil({@link #getNumIdle}/
	 * abs({@link #getNumTestsPerEvictionRun}))</code> which means that when the value is
	 * {@code -n} roughly one nth of the idle objects will be tested per run.
	...
	/**
	 * Sets the maximum number of objects to examine during each run (if any) of the idle
	 * object evictor thread. When positive, the number of tests performed for a run will
	 * be the minimum of the configured value and the number of idle instances in the
	 * pool. When negative, the number of tests performed will be
	 * <code>ceil({@link #getNumIdle}/
	...
	/**
	 * Gets whether a check is made for abandoned objects when an object is borrowed from
	 * this pool.
	 ...
	 * @return {@code true} if abandoned object removal is configured to be activated when
	 * the evictor runs otherwise {@code false}
	...
	/**
	 * Gets the timeout before which an object will be considered to be abandoned by this
	 * pool.
	 * @return The abandoned object timeout in seconds if abandoned object removal is
	 * configured for this pool; Integer.MAX_VALUE otherwise.
	...
	/**
	 * Gets the total number of objects returned to this pool over the lifetime of the
	 * pool. This excludes attempts to return the same object multiple times.
	 ...
	 * Gets the minimum amount of time an object may sit idle in the pool before it is
	 * eligible for eviction by the idle object evictor (if any - see
	 * {@link #setTimeBetweenEvictionRuns(Duration)}), with the extra condition that at
	 * least {@code minIdle} object instances remain in the pool. This setting is
	 * overridden by {@link #getMinEvictableIdleTime} (that is, if
	 * {@link #getMinEvictableIdleTime} is positive, then
	 * {@link #getSoftMinEvictableIdleTime} is ignored).
	 * @return minimum amount of time an object may sit idle in the pool before it is
	 * eligible for eviction if minIdle instances are available
	 ...
	 * {@link #getMinEvictableIdleTime} is positive, then
	 * {@link #getSoftMinEvictableIdleTime} is ignored).
	 * @param softMinEvictableIdleTime minimum amount of time an object may sit idle in
	 * the pool before it is eligible for eviction if minIdle instances are available
	...
	public final void setSoftMinEvictableIdleTime(final Duration softMinEvictableIdleTime) {
		this.softMinEvictableIdleDuration = PoolImplUtils.nonNull(softMinEvictableIdleTime,
	 ...
	 * Gets the minimum amount of time an object may sit idle in the pool before it is
	 * eligible for eviction by the idle object evictor (if any - see
	 * {@link #setTimeBetweenEvictionRunsMillis(long)}), with the extra condition that at
	 * least {@code minIdle} object instances remain in the pool. This setting is
	 * overridden by {@link #getMinEvictableIdleTimeMillis} (that is, if
	 ...
	 * in the pool before it is eligible for eviction if minIdle instances are available
	 * @see #getSoftMinEvictableIdleTimeMillis
	 * @deprecated Use {@link #setSoftMinEvictableIdle(Duration)}.
	 */
	@Deprecated
	...
	String getStatsString() {
		...
		return String
			.format("activeTimes=%s, blockWhenExhausted=%s, borrowedCount=%,d, closed=%s, createdCount=%,d, destroyedByBorrowValidationCount=%,d, "
					...
					+ "durationBetweenEvictionRuns=%s, waitTimes=%s", activeTimes.getCurrentValues(),
					blockWhenExhausted, borrowedCount.get(), closed, createdCount.get(),
					destroyedByBorrowValidationCount.get(), destroyedByEvictorCount.get(),
					evictorShutdownTimeoutDuration, fairness, idleTimes.getCurrentValues(), lifo,
					maxBorrowWaitDuration.get(), maxTotal, maxWaitDuration, minEvictableIdleDuration,
					numTestsPerEvictionRun, returnedCount, softMinEvictableIdleDuration, testOnBorrow, testOnCreate,
					testOnReturn, testWhileIdle, durationBetweenEvictionRuns, waitTimes.getCurrentValues());
	}

	/**
	 * Gets the listener used (if any) to receive notifications of exceptions unavoidably
	...
	/**
	 * Sets the listener used (if any) to receive notifications of exceptions unavoidably
	 * swallowed by the pool.
	...
	/**
	 * Gets whether objects borrowed from the pool will be validated before being returned
	 * from the {@code borrowObject()} method. Validation is performed by the
	 * {@code validateObject()} method of the factory associated with the pool. If the
	 * object fails to validate, it will be removed from the pool and destroyed, and a new
	 * attempt will be made to borrow an object from the pool.
	 * @return {@code true} if objects are validated before being returned from the
	...
	/**
	 * Sets whether objects borrowed from the pool will be validated before being returned
	 * from the {@code borrowObject()} method. Validation is performed by the
	 * {@code validateObject()} method of the factory associated with the pool. If the
	 * object fails to validate, it will be removed from the pool and destroyed, and a new
	 * attempt will be made to borrow an object from the pool.
	 * @param testOnBorrow {@code true} if objects should be validated before being
	...
	/**
	 * Gets whether objects created for the pool will be validated before being returned
	 * from the {@code borrowObject()} method. Validation is performed by the
	 * {@code validateObject()} method of the factory associated with the pool. If the
	 * object fails to validate, then {@code borrowObject()} will fail.
	 * @return {@code true} if newly created objects are validated before being returned
	...
	/**
	 * Sets whether objects created for the pool will be validated before being returned
	 * from the {@code borrowObject()} method. Validation is performed by the
	 * {@code validateObject()} method of the factory associated with the pool. If the
	 * object fails to validate, then {@code borrowObject()} will fail.
	 * @param testOnCreate {@code true} if newly created objects should be validated
	 * before being returned from the {@code borrowObject()} method
	 * @see #getTestOnCreate
	 * @since 2.2
	 */
	...
	/**
	 * Gets whether objects borrowed from the pool will be validated when they are
	 * returned to the pool via the {@code returnObject()} method. Validation is performed
	 * by the {@code validateObject()} method of the factory associated with the pool.
	 * Returning objects that fail validation are destroyed rather then being returned the
	 * pool.
	 * @return {@code true} if objects are validated on return to the pool via the
	...
	/**
	 * Sets whether objects borrowed from the pool will be validated when they are
	 * returned to the pool via the {@code returnObject()} method. Validation is performed
	 * by the {@code validateObject()} method of the factory associated with the pool.
	 * Returning objects that fail validation are destroyed rather then being returned the
	 * pool.
	 * @param testOnReturn {@code true} if objects are validated on return to the pool via
	 * the {@code returnObject()} method
	 * @see #getTestOnReturn
	 */
	...
	/**
	 * Gets whether objects sitting idle in the pool will be validated by the idle object
	 * evictor (if any - see {@link #setTimeBetweenEvictionRuns(Duration)}). Validation is
	 * performed by the {@code validateObject()} method of the factory associated with the
	 * pool. If the object fails to validate, it will be removed from the pool and
	 * destroyed.
	...
	/**
	 * Sets whether objects sitting idle in the pool will be validated by the idle object
	 * evictor (if any - see {@link #setTimeBetweenEvictionRuns(Duration)}). Validation is
	 * performed by the {@code validateObject()} method of the factory associated with the
	 * pool. If the object fails to validate, it will be removed from the pool and
	 * destroyed. Note that setting this property has no effect unless the idle object
	 * evictor is enabled by setting {@code timeBetweenEvictionRunsMillis} to a positive
	 * value.
	...
	public final void setTimeBetweenEvictionRuns(final Duration timeBetweenEvictionRuns) {
		this.durationBetweenEvictionRuns = PoolImplUtils.nonNull(timeBetweenEvictionRuns,
				BaseObjectPoolConfig.DEFAULT_TIME_BETWEEN_EVICTION_RUNS);
		startEvictor(this.durationBetweenEvictionRuns);
	}

	/**
	 * Gets the number of milliseconds to sleep between runs of the idle object evictor
	 ...
	 * <li>When non-positive, no idle object evictor thread runs.</li>
	 * </ul>
	 * @param timeBetweenEvictionRunsMillis number of milliseconds to sleep between
	 * evictor runs
	 * @see #getDurationBetweenEvictionRuns()
	...
	/**
	 * Registers the pool with the platform MBean server. The registered name will be
	 * {@code jmxNameBase + jmxNamePrefix + i} where i is the least integer greater than
	 * or equal to 1 such that the name is not already registered. Swallows
	 * MBeanRegistrationException, NotCompliantMBeanException returning null.
	 * @param config Pool configuration
	 * @param jmxNameBase default base JMX name for this pool
	 * @param jmxNamePrefix name prefix
	 * @return registered ObjectName, null if registration fails
	 */
	private ObjectName jmxRegister(final BaseObjectPoolConfig<T> config, final String jmxNameBase,
	 ...
	 * @throws ExceptionInInitializerError if the initialization provoked by this method
	 * fails
	 * @throws ClassNotFoundException if the class cannot be located by the specified
	 * class loader
	 * @throws IllegalAccessException if this {@code Constructor} object is enforcing Java
	 * language access control and the underlying constructor is inaccessible.
	 * @throws IllegalArgumentException if the number of actual and formal parameters
	 ...
	 * formal parameter type by a method invocation conversion; if this constructor
	 ...
	 * @throws SecurityException If a security manage is present and the caller's class
	 * loader is not the same as or an ancestor of the class loader for the current class
	 * and invocation of {@link SecurityManager#checkPackageAccess s.checkPackageAccess()}
	 * denies access to the package of this class.
	 */
	@SuppressWarnings("unchecked")
	private void setEvictionPolicy(final String className, final ClassLoader classLoader) throws ClassNotFoundException,
	...
	/**
	 * Sets the name of the {@link EvictionPolicy} implementation that is used by this
	 * pool. The Pool will attempt to load the class using the given class loader. If that
	 * fails, use the class loader for the {@link EvictionPolicy} interface.
	 * @param evictionPolicyClassName the fully qualified class name of the new eviction
	 * policy
	 * @param classLoader the class loader to load the given
	 * {@code evictionPolicyClassName}.
	 * @see #getEvictionPolicyClassName()
	 * @since 2.6.0 If loading the class using the given class loader fails, use the class
	...
	/**
	 * Sets the maximum duration the {@code borrowObject()} method should block before
	 * throwing an exception when the pool is exhausted and {@link #getBlockWhenExhausted}
	 * is true. When less than 0, the {@code borrowObject()} method may block
	 * indefinitely.
	 * @param maxWaitDuration the maximum duration {@code borrowObject()} will block or
	...
	/**
	 * <p>
	 * Starts the evictor with the given delay. If there is an evictor running when this
	 * method is called, it is stopped and replaced with a new evictor with the specified
	 * delay.
	 * </p>
	 *
	 * <p>
	...
	final void startEvictor(final Duration delay) {
		synchronized (evictionLock) {
			...
			if (evictor == null) { // Starting evictor for the first time or after a
									// cancel
				...
				synchronized (EvictionTimer.class) { // Ensure no cancel can happen
														// between cancel / schedule calls
	...
	/**
	 * Swallows an exception and notifies the configured listener for swallowed exceptions
	 * queue.
	 ...
	 * @param activeTime the amount of time (in milliseconds) that the returning object
	 * was checked out
	 */
	 ...
	 * <p>
	 * GenericObjectPool and GenericKeyedObjectPool maintain references to all objects
	 * under management using maps keyed on the objects. This wrapper class ensures that
	 * objects can work as hash keys.
	 *
	...
	/**
	 * Maintains a cache of values for a single metric and reports statistics on the
	 * cached values.
	 */
	private static class StatsStore {
		...
		/**
		 * Adds a value to the cache. If the cache is full, one of the existing values is
	...
	class Evictor implements Runnable {
		...
		/**
		 * Run pool maintenance. Evict objects qualifying for eviction and then ensure
		 * that the minimum number of idle instances are available. Since the Timer that
		 * invokes Evictors is shared for all Pools but pools may exist in different class
		 * loaders, the Evictor ensures that any actions taken are under the class loader
		 * of the factory associated with the pool.
		 */
		@Override
		public void run() {
