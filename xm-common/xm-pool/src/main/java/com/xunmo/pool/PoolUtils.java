...
public final class PoolUtils {
	...
	/**
	 * PoolUtils instances should NOT be constructed in standard programming. Instead, the
	 * class should be used procedurally: PoolUtils.adapt(aPool);. This constructor is
	 ...
	 * {@code Collection keys} in the keyedPool. At most one idle object will be added per
	 * period.
	 ...
	 * @param periodMillis the frequency in milliseconds to check the number of idle
	 * objects in a keyedPool, see {@link Timer#schedule(TimerTask, long, long)}.
	 * @param <K> the type of the pool key
	 * @param <V> the type of pool entries
	 * @param <E> the type of exception thrown by a pool
	 * @return a {@link Map} of key and {@link TimerTask} pairs that will periodically
	 * check the pools idle object count.
	 * @throws IllegalArgumentException when {@code keyedPool}, {@code keys}, or any of
	 * the values in the collection is {@code null} or when {@code minIdle} is negative or
	 * when {@code period} isn't valid for {@link Timer#schedule(TimerTask, long, long)}.
	 * @see #checkMinIdle(KeyedObjectPool, Object, int, long)
	 */
	public static <K, V, E extends Exception> Map<K, TimerTask> checkMinIdle(final KeyedObjectPool<K, V, E> keyedPool,
			final Collection<K> keys, final int minIdle, final long periodMillis) throws IllegalArgumentException {
		if (keys == null) {
	...
	/**
	 * Periodically check the idle object count for the key in the keyedPool. At most one
	 * idle object will be added per period. If there is an exception when calling
	 * {@link KeyedObjectPool#addObject(Object)} then no more checks for that key will be
	 * performed.
	 * @param keyedPool the keyedPool to check periodically.
	 ...
	 * @param <V> the type of pool entries
	 * @param <E> the type of exception thrown by a pool
	 * @return the {@link TimerTask} that will periodically check the pools idle object
	 * count.
	 * @throws IllegalArgumentException when {@code keyedPool}, {@code key} is
	 * {@code null} or when {@code minIdle} is negative or when {@code period} isn't valid
	 * for {@link Timer#schedule(TimerTask, long, long)}.
	 */
	public static <K, V, E extends Exception> TimerTask checkMinIdle(final KeyedObjectPool<K, V, E> keyedPool,
			final K key, final int minIdle, final long periodMillis) throws IllegalArgumentException {
		if (keyedPool == null) {
	 ...
	 * be added per period. If there is an exception when calling
	 ...
	 * idle object.
	 * @param periodMillis the frequency in milliseconds to check the number of idle
	 * objects in a pool, see {@link Timer#schedule(TimerTask, long, long)}.
	 * @param <T> the type of objects in the pool
	 * @param <E> type of exceptions from the pool
	 * @return the {@link TimerTask} that will periodically check the pools idle object
	 * count.
	 * @throws IllegalArgumentException when {@code pool} is {@code null} or when
	 * {@code minIdle} is negative or when {@code period} isn't valid for
	 * {@link Timer#schedule(TimerTask, long, long)}
	 */
	public static <T, E extends Exception> TimerTask checkMinIdle(final ObjectPool<T, E> pool, final int minIdle,
			final long periodMillis) throws IllegalArgumentException {
		if (pool == null) {
			throw new IllegalArgumentException(MSG_NULL_KEYED_POOL);
		}
		if (minIdle < 0) {
	...
	/**
	 * Should the supplied Throwable be re-thrown (eg if it is an instance of one of the
	 * Throwables that should never be swallowed). Used by the pool error handling for
	 * operations that throw exceptions that normally need to be ignored.
	...
	/**
	 * Returns a pool that adaptively decreases its size when idle objects are no longer
	 * needed. This is intended as an always thread-safe alternative to using an idle
	 * object evictor provided by many pool implementations. This is also an effective way
	 * to shrink FIFO ordered pools that experience load spikes.
	 * @param keyedPool the KeyedObjectPool to be decorated so it shrinks its idle count
	 * when possible.
	 * @param <K> the type of the pool key
	 * @param <V> the type of pool entries
	 * @param <E> the type of exception thrown by a pool
	 * @return a pool that adaptively decreases its size when idle objects are no longer
	 * needed.
	 * @throws IllegalArgumentException when {@code keyedPool} is {@code null}.
	...
	public static <K, V, E extends Exception> KeyedObjectPool<K, V, E> erodingPool(
			final KeyedObjectPool<K, V, E> keyedPool) {
	 ...
	 * to shrink FIFO ordered pools that experience load spikes.
	 * <p>
	 * The factor parameter provides a mechanism to tweak the rate at which the pool tries
	 * to shrink its size. Values between 0 and 1 cause the pool to try to shrink its size
	 * more often. Values greater than 1 cause the pool to less frequently try to shrink
	 * its size.
	 * </p>
	 * @param keyedPool the KeyedObjectPool to be decorated so it shrinks its idle count
	 * when possible.
	 * @param factor a positive value to scale the rate at which the pool tries to reduce
	 * its size. If 0 &lt; factor &lt; 1 then the pool shrinks more aggressively. If 1
	 * &lt; factor then the pool shrinks less aggressively.
	 * @param <K> the type of the pool key
	 * @param <V> the type of pool entries
	 * @param <E> the type of exception thrown by a pool
	 * @return a pool that adaptively decreases its size when idle objects are no longer
	 * needed.
	 * @throws IllegalArgumentException when {@code keyedPool} is {@code null} or when
	 * {@code factor} is not positive.
	 * @see #erodingPool(KeyedObjectPool, float, boolean)
	 */
	public static <K, V, E extends Exception> KeyedObjectPool<K, V, E> erodingPool(
	 ...
	 * </p>
	 * <p>
	 * The perKey parameter determines if the pool shrinks on a whole pool basis or a per
	 * key basis. When perKey is false, the keys do not have an effect on the rate at
	 * which the pool tries to shrink its size. When perKey is true, each key is shrunk
	 * independently.
	 * </p>
	 * @param keyedPool the KeyedObjectPool to be decorated so it shrinks its idle count
	 * when possible.
	 ...
	 */
	public static <K, V, E extends Exception> KeyedObjectPool<K, V, E> erodingPool(
			final KeyedObjectPool<K, V, E> keyedPool, final float factor, final boolean perKey) {
		if (keyedPool == null) {
			throw new IllegalArgumentException(MSG_NULL_KEYED_POOL);
		}
		if (factor <= 0f) {
	 ...
	 * object evictor provided by many pool implementations. This is also an effective way
	 * to shrink FIFO ordered pools that experience load spikes.
	 * @param pool the ObjectPool to be decorated so it shrinks its idle count when
	 * possible.
	 * @param <T> the type of objects in the pool
	 * @param <E> type of exceptions from the pool
	 * @return a pool that adaptively decreases its size when idle objects are no longer
	...
	/**
	 * Calls {@link KeyedObjectPool#addObject(Object)} on {@code keyedPool} with each key
	 * in {@code keys} for {@code count} number of times. This has the same effect as
	 * calling {@link #prefill(KeyedObjectPool, Object, int)} for each key in the
	 * {@code keys} collection.
	 ...
	 * @param <V> the type of pool entries
	 * @param <E> the type of exception thrown by a pool
	 * @throws E when {@link KeyedObjectPool#addObject(Object)} fails.
	...
	public static <K, V, E extends Exception> void prefill(final KeyedObjectPool<K, V, E> keyedPool,
			final Collection<K> keys, final int count) throws E, IllegalArgumentException {
		if (keys == null) {
			throw new IllegalArgumentException(MSG_NULL_KEYS);
		}
	 ...
	 * @param <E> the type of exception thrown by a pool
	 * @throws E when {@link KeyedObjectPool#addObject(Object)} fails.
	 * @throws IllegalArgumentException when {@code keyedPool} or {@code key} is
	 * {@code null}.
	 * @deprecated Use {@link KeyedObjectPool#addObjects(Object, int)}.
	 */
	@Deprecated
	public static <K, V, E extends Exception> void prefill(final KeyedObjectPool<K, V, E> keyedPool, final K key,
			final int count) throws E, IllegalArgumentException {
		if (keyedPool == null) {
			throw new IllegalArgumentException(MSG_NULL_KEYED_POOL);
		}
	...
	/**
	 * Calls {@link ObjectPool#addObject()} on {@code pool} {@code count} number of times.
	 * @param pool the pool to prefill.
	 * @param count the number of idle objects to add.
	 * @param <T> the type of objects in the pool
	 * @param <E> type of exceptions from the pool
	 * @throws E when {@link ObjectPool#addObject()} fails.
	 * @throws IllegalArgumentException when {@code pool} is {@code null}.
	...
	/**
	 * Returns a synchronized (thread-safe) KeyedPooledObjectFactory backed by the
	 * specified KeyedPooledObjectFactory.
	 * @param keyedFactory the KeyedPooledObjectFactory to be "wrapped" in a synchronized
	 ...
	 * KeyedObjectPool.
	 * <p>
	 * <b>Note:</b> This should not be used on pool implementations that already provide
	 * proper synchronization such as the pools provided in the Commons Pool library.
	 * Wrapping a pool that {@link #wait() waits} for poolable objects to be returned
	 * before allowing another one to be borrowed with another layer of synchronization
	 * will cause liveliness issues or a deadlock.
	 * </p>
	 ...
	 */
	public static <K, V, E extends Exception> KeyedObjectPool<K, V, E> synchronizedPool(
			final KeyedObjectPool<K, V, E> keyedPool) {
		/*
		 * assert !(keyedPool instanceof GenericKeyedObjectPool) :
		 * "GenericKeyedObjectPool is already thread-safe"; assert !(keyedPool instanceof
		 * StackKeyedObjectPool) : "StackKeyedObjectPool is already thread-safe"; assert
	...
	public static <T, E extends Exception> ObjectPool<T, E> synchronizedPool(final ObjectPool<T, E> pool) {
		 ...
		 * SoftReferenceObjectPool) : "SoftReferenceObjectPool is already thread-safe";
	...
	/**
	 * Returns a synchronized (thread-safe) PooledObjectFactory backed by the specified
	 * PooledObjectFactory.
	 ...
	 */
	public static <T, E extends Exception> PooledObjectFactory<T, E> synchronizedPooledFactory(
			final PooledObjectFactory<T, E> factory) {
		return new SynchronizedPooledObjectFactory<>(factory);
	}

	/**
	 * Encapsulate the logic for when the next poolable object should be discarded. Each
	 * time update is called, the next time to shrink is recomputed, based on the float
	 * factor, number of idle instances in the pool and high water mark. Float factor is
	 * assumed to be between 0 and 1. Values closer to 1 cause less frequent erosion
	 * events. Erosion event timing also depends on numIdle. When this value is relatively
	 * high (close to previously established high water mark), erosion occurs more
	 * frequently.
	 */
	...
	/**
	 * Decorates a keyed object pool, adding "eroding" behavior. Based on the configured
	 * erosion factor, objects returning to the pool may be invalidated instead of being
	 * added to idle capacity.
	 *
	...
	private static class ErodingKeyedObjectPool<K, V, E extends Exception> implements KeyedObjectPool<K, V, E> {
		...
		/**
		 * Creates an ErodingObjectPool wrapping the given pool using the specified
		 * erosion factor.
		 ...
		 * @see #erodingFactor
		 */
		protected ErodingKeyedObjectPool(final KeyedObjectPool<K, V, E> keyedPool, final ErodingFactor erodingFactor) {
			if (keyedPool == null) {
				throw new IllegalArgumentException(MSG_NULL_KEYED_POOL);
			}
		 ...
		 * @see #erodingFactor
		 */
		public ErodingKeyedObjectPool(final KeyedObjectPool<K, V, E> keyedPool, final float factor) {
		...
		/**
		 * Returns obj to the pool, unless erosion is triggered, in which case obj is
		 * invalidated. Erosion is triggered when there are idle instances in the pool
		 * associated with the given key and more than the configured
		 * {@link #erodingFactor erosion factor} time has elapsed since the last
		 * returnObject activation.
	...
	/**
	 * Decorates an object pool, adding "eroding" behavior. Based on the configured
	 * {@link #factor erosion factor}, objects returning to the pool may be invalidated
	 * instead of being added to idle capacity.
	 *
	...
	private static class ErodingObjectPool<T, E extends Exception> implements ObjectPool<T, E> {
		...
		/**
		 * Returns * Gets obj to the pool, unless erosion is triggered, in which case obj
		 * is invalidated. Erosion is triggered when there are idle instances in the pool
		 * and more than the {@link #factor erosion factor}-determined time has elapsed
	 ...
	 * Timing of erosion events is tracked separately for separate keyed pools.
	 *
	 * @param <K> object pool key type
	 * @param <V> object pool value type
	 * @param <E> exception thrown by this pool
	 */
	private static final class ErodingPerKeyKeyedObjectPool<K, V, E extends Exception>
			extends ErodingKeyedObjectPool<K, V, E> {
		...
		/**
		 * Creates a new ErordingPerKeyKeyedObjectPool decorating the given keyed pool
		 * with the specified erosion factor.
	...
	/**
	 * Timer task that adds objects to the pool until the number of idle instances for the
	 * given key reaches the configured minIdle. Note that this is not the same as the
	 * pool's minIdle setting.
	 *
	 * @param <K> object pool key type
	 * @param <V> object pool value type
	 * @param <E> exception thrown by this pool
	...
	private static final class KeyedObjectPoolMinIdleTimerTask<K, V, E extends Exception> extends TimerTask {
		 ...
		 */
		KeyedObjectPoolMinIdleTimerTask(final KeyedObjectPool<K, V, E> keyedPool, final K key, final int minIdle)
	...
	/**
	 * Timer task that adds objects to the pool until the number of idle instances reaches
	 * the configured minIdle. Note that this is not the same as the pool's minIdle
	 * setting.
	 *
	 * @param <T> type of objects in the pool
	 * @param <E> type of exceptions from the pool
	...
	private static final class ObjectPoolMinIdleTimerTask<T, E extends Exception> extends TimerTask {
		...
		/**
		 * Constructs a new ObjectPoolMinIdleTimerTask for the given pool with the given
		 * minIdle setting.
		 * @param pool object pool
		 * @param minIdle number of idle instances to maintain
		 * @throws IllegalArgumentException if the pool is null
		 */
		ObjectPoolMinIdleTimerTask(final ObjectPool<T, E> pool, final int minIdle) throws IllegalArgumentException {
			if (pool == null) {
				throw new IllegalArgumentException(MSG_NULL_POOL);
			}
			this.pool = pool;
	...
	static final class SynchronizedKeyedObjectPool<K, V, E extends Exception> implements KeyedObjectPool<K, V, E> {
		 ...
		 */
		SynchronizedKeyedObjectPool(final KeyedObjectPool<K, V, E> keyedPool) throws IllegalArgumentException {
			if (keyedPool == null) {
				throw new IllegalArgumentException(MSG_NULL_KEYED_POOL);
			}
			this.keyedPool = keyedPool;
	...
	/**
	 * A fully synchronized KeyedPooledObjectFactory that wraps a KeyedPooledObjectFactory
	 * and synchronizes access to the wrapped factory methods.
	 * <p>
	 * <b>Note:</b> This should not be used on pool implementations that already provide
	 * proper synchronization such as the pools provided in the Commons Pool library.
	 * </p>
	 ...
	 */
	private static final class SynchronizedKeyedPooledObjectFactory<K, V, E extends Exception>
		 ...
		 */
		SynchronizedKeyedPooledObjectFactory(final KeyedPooledObjectFactory<K, V, E> keyedFactory)
				throws IllegalArgumentException {
	...
	private static final class SynchronizedObjectPool<T, E extends Exception> implements ObjectPool<T, E> {
		 ...
		 * @throws IllegalArgumentException if the pool is null
		 */
		SynchronizedObjectPool(final ObjectPool<T, E> pool) throws IllegalArgumentException {
			if (pool == null) {
				throw new IllegalArgumentException(MSG_NULL_POOL);
			}
			this.pool = pool;
	 ...
	 */
	private static final class SynchronizedPooledObjectFactory<T, E extends Exception>
		 ...
		 * @throws IllegalArgumentException if the factory is null
		 */
		SynchronizedPooledObjectFactory(final PooledObjectFactory<T, E> factory) throws IllegalArgumentException {
			if (factory == null) {
	...
	/**
	 * Timer used to periodically check pools idle object count. Because a {@link Timer}
	 * creates a {@link Thread}, an IODH is used.
	 */
