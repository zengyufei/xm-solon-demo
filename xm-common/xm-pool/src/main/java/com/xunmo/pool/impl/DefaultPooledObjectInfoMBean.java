...
/**
 * The interface that defines the information about pooled objects that will be exposed
 * via JMX.
 * <p>
 * NOTE: This interface exists only to define those attributes and methods that will be
 * made available via JMX. It must not be implemented by clients as it is subject to
 * change between major, minor and patch version releases of commons pool. Clients that
 * implement this interface may not, therefore, be able to upgrade to a new minor or patch
...
public interface DefaultPooledObjectInfoMBean {
	...
	/**
	 * Gets the time (using the same basis as {@link java.time.Clock#instant()}) that
	 * pooled object was created.
	...
	/**
	 * Gets the time (using the same basis as {@link java.time.Clock#instant()}) the
	 * polled object was last borrowed.
	...
	/**
	 * Gets the time (using the same basis as {@link java.time.Clock#instant()})the
	 * wrapped object was last returned.
