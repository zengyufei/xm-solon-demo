...
import com.xunmo.pool.TrackedUse;
import com.xunmo.pool.UsageTracking;
...
public class AbandonedConfig {
	...
	/**
	 * Determines whether or not to log stack traces for application code which abandoned
	 * an object.
	 */
	private boolean logAbandoned;

	/**
	 * Determines whether or not to log full stack traces when logAbandoned is true. If
	 * disabled, then a faster method for logging stack traces with only class data may be
	 * used if possible.
	 *
	 * @since 2.5
	 */
	...
	/**
	 * PrintWriter to use to log information on abandoned objects. Use of default system
	...
	/**
	 * If the pool implements {@link UsageTracking}, should the pool record a stack trace
	 * every time a method is called on a pooled object and retain the most recent stack
	 * trace to aid debugging of abandoned objects?
	 */
	 ...
	 * <p>
	 * Defaults to false. Logging of abandoned objects adds overhead for every object
	 * created because a stack trace has to be generated.
	 * @return boolean true if stack trace logging is turned on for abandoned objects
	 */
	...
	/**
	 * Gets the log writer being used by this configuration to log information on
	 * abandoned objects. If not set, a PrintWriter based on System.out with the system
	...
	/**
	 * Sets the log writer to be used by this configuration to log information on
	 * abandoned objects.
	...
	/**
	 * <p>
	 * Flag to remove abandoned objects if they exceed the removeAbandonedTimeout when
	 * borrowObject is invoked.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * If set to true, abandoned objects are removed by borrowObject if there are fewer
	 * than 2 idle objects available in the pool and
	 * {@code getNumActive() &gt; getMaxTotal() - 3}
	 * </p>
	 ...
	 * Flag to remove abandoned objects if they exceed the removeAbandonedTimeout when
	 * borrowObject is invoked.
	 * @param removeAbandonedOnBorrow true means abandoned objects will be removed by
	 * borrowObject
	 ...
	 * <p>
	 * Flag to remove abandoned objects if they exceed the removeAbandonedTimeout when
	 * pool maintenance (the "evictor") runs.
	 * </p>
	 *
	 * <p>
	 * The default value is false.
	 ...
	 *
	 * <p>
	 * If set to true, abandoned objects are removed by the pool maintenance thread when
	 * it runs. This setting has no effect unless maintenance is enabled by setting
	 * {@link GenericObjectPool#getDurationBetweenEvictionRuns()
	 * durationBetweenEvictionRuns} to a positive number.
	 * </p>
	...
	/**
	 * Flag to remove abandoned objects if they exceed the removeAbandonedTimeout when
	 * pool maintenance runs.
	 * @param removeAbandonedOnMaintenance true means abandoned objects will be removed by
	 * pool maintenance
	 ...
	 *
	 * <p>
	 * Setting this property has no effect if {@link #getRemoveAbandonedOnBorrow()
	 * removeAbandonedOnBorrow} and {@link #getRemoveAbandonedOnMaintenance()
	 * removeAbandonedOnMaintenance} are both false.
	 * </p>
	...
	public void setRemoveAbandonedTimeout(final Duration removeAbandonedTimeout) {
		this.removeAbandonedTimeoutDuration = PoolImplUtils.nonNull(removeAbandonedTimeout,
	...
	/**
	 * Indicates if full stack traces are required when {@link #getLogAbandoned()
	 * logAbandoned} is true. Defaults to true. Logging of abandoned objects requiring a
	 * full stack trace will generate an entire stack trace to generate for every object
	 * created. If this is disabled, a faster but less informative stack walking mechanism
	 * may be used if available.
	 * @return true if full stack traces are required for logging abandoned connections,
	 * or false if abbreviated stack traces are acceptable
	 * @see CallStack
	 * @since 2.5
	 */
	...
	/**
	 * Sets the flag to require full stack traces for logging abandoned connections when
	 * enabled.
	 * @param requireFullStackTrace indicates whether or not full stack traces are
	 * required in abandoned connection logs
	 * @see CallStack
	...
	/**
	 * If the pool implements {@link UsageTracking}, configure whether the pool should
	 * record a stack trace every time a method is called on a pooled object and retain
	 * the most recent stack trace to aid debugging of abandoned objects.
	 * @param useUsageTracking A value of {@code true} will enable the recording of a
	 * stack trace on every use of a pooled object
	 */
