...
/**
 * An optionally-bounded {@linkplain java.util.concurrent.BlockingDeque blocking deque}
 * based on linked nodes.
 *
 * <p>
 * The optional capacity bound constructor argument serves as a way to prevent excessive
 * expansion. The capacity, if unspecified, is equal to {@link Integer#MAX_VALUE}. Linked
 * nodes are dynamically created upon each insertion unless this would bring the deque
 * above capacity.
 * </p>
 *
 * <p>
 * Most operations run in constant time (ignoring time spent blocking). Exceptions include
 * {@link #remove(Object) remove}, {@link #removeFirstOccurrence removeFirstOccurrence},
 * {@link #removeLastOccurrence removeLastOccurrence}, {@link #contains contains},
 * {@link #iterator iterator.remove()}, and the bulk operations, all of which run in
 * linear time.
 * </p>
 *
 * <p>
 ...
 * <a href="{@docRoot}/../technotes/guides/collections/index.html"> Java Collections
 * Framework</a>.
 * </p>
 *
 * @param <E> the type of elements held in this collection
 * <p>
 * Note: This was copied from Apache Harmony and modified to suit the needs of Commons
 * Pool.
 * @since 2.0
 */
class LinkedBlockingDeque<E> extends AbstractQueue<E> implements Deque<E>, Serializable {
	...
	/*
	 * Implemented as a simple doubly-linked list protected by a single lock and using
	 * conditions to manage blocking.
	 *
	 * To implement weakly consistent iterators, it appears we need to keep all Nodes
	 * GC-reachable from a predecessor dequeued Node. That would cause two problems: -
	 * allow a rogue Iterator to cause unbounded memory retention - cause
	 * cross-generational linking of old Nodes to new Nodes if a Node was tenured while
	 * live, which generational GCs have a hard time dealing with, causing repeated major
	 * collections. However, only non-deleted Nodes need to be reachable from dequeued
	 * Nodes, and reachability does not necessarily have to be of the kind understood by
	 * the GC. We use the trick of linking a Node that has just been dequeued to itself.
	 * Such a self-link implicitly means to jump to "first" (for next links) or "last"
	 * (for prev links).
	 */

	/*
	 * We have "diamond" multiple interface/abstract class inheritance here, and that
	 * introduces ambiguities. Often we want the BlockingDeque javadoc combined with the
	 * AbstractQueue implementation, so a lot of method specs are duplicated here.
	 */
	...
	/**
	 * Creates a {@code LinkedBlockingDeque} with a capacity of {@link Integer#MAX_VALUE}
	 * and the given fairness policy.
	 * @param fairness true means threads waiting on the deque should be served as if
	 * waiting in a FIFO request queue
	 */
	...
	/**
	 * Creates a {@code LinkedBlockingDeque} with a capacity of {@link Integer#MAX_VALUE},
	 * initially containing the elements of the given collection, added in traversal order
	...
	/**
	 * Creates a {@code LinkedBlockingDeque} with the given (fixed) capacity and fairness
	 * policy.
	 * @param capacity the capacity of this deque
	 * @param fairness true means threads waiting on the deque should be served as if
	 * waiting in a FIFO request queue
	...
	/**
	 * Returns {@code true} if this deque contains the specified element. More formally,
	 * returns {@code true} if and only if this deque contains at least one element
	 ...
	 * @throws UnsupportedOperationException if the add operation is not supported by the
	 * specified collection
	 * @throws ClassCastException if the class of the elements held by this collection
	 * prevents them from being added to the specified collection
	 ...
	 * This method differs from {@link #peek peek} only in that it throws an exception if
	 * this deque is empty.
	 *
	 * <p>
	...
	/**
	 * Returns true if there are threads waiting to take instances from this deque. See
	 * disclaimer on accuracy in
	 * {@link java.util.concurrent.locks.ReentrantLock#hasWaiters(Condition)}.
	 * @return true if there is at least one thread waiting on this deque's notEmpty
	 * condition.
	 */
	...
	/**
	 * Interrupts the threads currently waiting to take an object from the pool. See
	 * disclaimer on accuracy in
	 ...
	 * elements will be returned in order from first (head) to last (tail). The returned
	 ...
	 * iterator, and may (but is not guaranteed to) reflect any modifications subsequent
	 * to construction.
	...
	/**
	 * Links the provided element as the last in the queue, waiting up to the specified
	 * time to do so if the queue is full.
	 * <p>
	...
	/**
	 * Links the provided element as the first in the queue, waiting up to the specified
	 * time to do so if the queue is full.
	 * @param e element to link
	 * @param timeout length of time to wait
	 * @return {@code true} if successful, otherwise {@code false}
	...
	/**
	 * Unlinks the first element in the queue, waiting up to the specified time to do so
	...
	/**
	 * Unlinks the last element in the queue, waiting up to the specified time to do so if
	 * the queue is empty.
	 * @param timeout length of time to wait
	 * @return the unlinked element
	 * @throws InterruptedException if the current thread is interrupted
	...
	/**
	 * Links the provided element as the last in the queue, waiting until there is space
	 * to do so if the queue is full.
	 *
	 * <p>
	...
	/**
	 * Links the provided element as the first in the queue, waiting until there is space
	 * to do so if the queue is full.
	 * @param e element to link
	 * @throws NullPointerException if e is null
	 * @throws InterruptedException if the thread is interrupted whilst waiting for space
	 ...
	 * absence of memory or resource constraints) accept without blocking. This is always
	 * equal to the initial capacity of this deque less the current {@code size} of this
	 * deque.
	 *
	 * <p>
	 * Note that you <em>cannot</em> always tell if an attempt to insert an element will
	 * succeed by inspecting {@code remainingCapacity} because it may be the case that
	...
	/**
	 * Retrieves and removes the head of the queue represented by this deque. This method
	 * differs from {@link #poll poll} only in that it throws an exception if this deque
	 * is empty.
	 *
	 * <p>
	...
	/**
	 * Removes the first occurrence of the specified element from this deque. If the deque
	 * does not contain the element, it is unchanged. More formally, removes the first
	 * element {@code e} such that {@code o.equals(e)} (if such an element exists).
	 ...
	 * threads a chance to interact with the collection, especially when count is close to
	 * capacity.
	 */
	...
	/**
	 * Unlinks the first element in the queue, waiting until there is an element to unlink
	 * if the queue is empty.
	 *
	 * <p>
	...
	/**
	 * Unlinks the last element in the queue, waiting until there is an element to unlink
	 * if the queue is empty.
	 * @return the unlinked element
	 * @throws InterruptedException if the current thread is interrupted
	 */
	 ...
	 * this deque. (In other words, this method must allocate a new array). The caller is
	 ...
	 * {@code OutputStream}
	 * @serialData The capacity (int), followed by elements (each an {@code Object}) in
	...
	private abstract class AbstractItr implements Iterator<E> {
		 ...
		 * in hasNext(), we must return item read under lock (in advance()) even if it was
