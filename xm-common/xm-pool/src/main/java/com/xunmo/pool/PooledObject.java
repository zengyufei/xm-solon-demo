...
/**
 * Defines the wrapper that is used to track the additional information, such as state,
 * for the pooled objects.
 * <p>
...
public interface PooledObject<T> extends Comparable<PooledObject<T>> {
	...
	/**
	 * Orders instances based on idle time - i.e. the length of time since the instance
	 * was returned to the pool. Used by the GKOP idle object evictor.
	 * <p>
	 * Note: This class has a natural ordering that is inconsistent with equals if
	 * distinct objects have the same identity hash code.
	 * </p>
	 * <p>
	 * {@inheritDoc}
	 * </p>
	...
	/**
	 * Deallocates the object and sets it {@link PooledObjectState#IDLE IDLE} if it is
	 * currently {@link PooledObjectState#ALLOCATED ALLOCATED}.
	...
	/**
	 * Gets the amount of time this object last spent in the active state (it may still be
	 * active in which case subsequent calls will return an increased value).
	...
	/**
	 * Gets the amount of time in milliseconds this object last spent in the active state
	 * (it may still be active in which case subsequent calls will return an increased
	 * value).
	...
	/**
	 * Gets the time (using the same basis as {@link Instant#now()}) that this object was
	 * created.
	...
	/**
	 * Gets the time (using the same basis as {@link java.time.Clock#instant()}) that this
	 * object was created.
	 * @return The creation time for the wrapped object.
	...
	/**
	 * Gets the amount of time that this object last spend in the idle state (it may still
	 * be idle in which case subsequent calls will return an increased value).
	...
	/**
	 * Gets the amount of time in milliseconds that this object last spend in the idle
	 * state (it may still be idle in which case subsequent calls will return an increased
	 * value).
	...
	/**
	 * Gets an estimate of the last time this object was used. If the class of the pooled
	 * object implements {@link TrackedUse}, what is returned is the maximum of
	 * {@link TrackedUse#getLastUsedInstant()} and {@link #getLastBorrowTime()}; otherwise
	 * this method gives the same value as {@link #getLastBorrowTime()}.
	...
	/**
	 * Prints the stack trace of the code that borrowed this pooled object and the stack
	 * trace of the last code to use this object (if available) to the supplied writer.
	 * @param writer The destination for the debug output.
	 */
	void printStackTrace(PrintWriter writer);

	/**
	 * Sets whether to use abandoned object tracking. If this is true the implementation
	 * will need to record the stack trace of the last caller to borrow this object.
	 * @param logAbandoned The new configuration setting for abandoned object tracking.
	 */
	void setLogAbandoned(boolean logAbandoned);

	/**
	 * Sets the stack trace generation strategy based on whether or not fully detailed
	 * stack traces are required. When set to false, abandoned logs may only include
	 * caller class information rather than method names, line numbers, and other normal
	 * metadata available in a full stack trace.
	...
	/**
	 * Attempts to place the pooled object in the {@link PooledObjectState#EVICTION}
	 * state.
	 * @return {@code true} if the object was placed in the
	 * {@link PooledObjectState#EVICTION} state otherwise {@code false}.
	 */
	boolean startEvictionTest();

	/**
	 * Gets a String form of the wrapper for debug purposes. The format is not fixed and
