...
import com.xunmo.pool.PooledObjectFactory;
...
class PoolImplUtils {
	...
	static Class<?> getFactoryType(final Class<? extends PooledObjectFactory> factoryClass) {
		...
		if (genericType instanceof Integer) {
			// POOL-324 org.apache.commons.pool2.impl.GenericObjectPool.getFactoryType()
			// throws
			// java.lang.ClassCastException
			//
			// A bit hackish, but we must handle cases when getGenericType() does not
			// return a concrete types.
	...
	/**
	 * Gets the concrete type used by an implementation of an interface that uses a
	 * generic type.
	...
	/**
	 * For a generic parameter, return either the Class used or if the type is unknown,
	 * the index for the type in definition of the class
	 * @param clazz defining class
	 * @param argType the type argument of interest
	 * @return An instance of {@link Class} representing the type used by the type
	 * parameter or an instance of {@link Integer} representing the index for the type in
	 * the definition of the defining class
	 */
	...
	/**
	 * Returns the greater of two {@code Instant} values. That is, the result is the
	 * argument closer to the value of {@link Instant#MAX}. If the arguments have the same
	...
	/**
	 * Returns the smaller of two {@code Instant} values. That is, the result is the
	 * argument closer to the value of {@link Instant#MIN}. If the arguments have the same
	 * value, the result is that same value.
	 * @param a an argument.
	 * @param b another argument.
	 ...
	 * @param amount the amount of the duration, measured in terms of the unit, positive
	 * or negative
