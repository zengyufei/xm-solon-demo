...
import com.xunmo.pool.BaseObjectPool;
import com.xunmo.pool.PoolUtils;
...
public class SoftReferenceObjectPool<T, E extends Exception> extends BaseObjectPool<T, E> {
	...
	/**
	 * Queue of broken references that might be able to be removed from {@code _pool}.
	 * This is used to help {@link #getNumIdle()} be more accurate with minimal
	 ...
	 * {@link PooledObjectFactory#validateObject( org.apache.commons.pool2.PooledObject)
	 * validated} and
	 * {@link PooledObjectFactory#passivateObject( org.apache.commons.pool2.PooledObject)
	 * passivated}. If validation fails, the new instance is
	 * {@link PooledObjectFactory#destroyObject( org.apache.commons.pool2.PooledObject)
	 * destroyed}. Exceptions generated by the factory {@code makeObject} or
	 * {@code passivate} are propagated to the caller. Exceptions destroying instances are
	 * silently swallowed.
	 * </p>
	 * @throws IllegalStateException if invoked on a {@link #close() closed} pool
	 * @throws E when the {@link #getFactory() factory} has a problem creating or
	...
	/**
	 * Borrows an object from the pool. If there are no idle instances available in the
	 * pool, the configured factory's {@link PooledObjectFactory#makeObject()} method is
	 * invoked to create a new instance.
	 * <p>
	 * All instances are
	 * {@link PooledObjectFactory#activateObject( org.apache.commons.pool2.PooledObject)
	 * activated} and
	 * {@link PooledObjectFactory#validateObject( org.apache.commons.pool2.PooledObject)
	 * validated} before being returned by this method. If validation fails or an
	 * exception occurs activating or validating an idle instance, the failing instance is
	 * {@link PooledObjectFactory#destroyObject( org.apache.commons.pool2.PooledObject)
	 * destroyed} and another instance is retrieved from the pool, validated and
	 * activated. This process continues until either the pool is empty or an instance
	 * passes validation. If the pool is empty on activation or it does not contain any
	 * valid instances, the factory's {@code makeObject} method is used to create a new
	 * instance. If the created instance either raises an exception on activation or fails
	 * validation, {@code NoSuchElementException} is thrown. Exceptions thrown by
	 * {@code MakeObject} are propagated to the caller; but other than {@code ThreadDeath}
	 * or {@code VirtualMachineError}, exceptions generated by activation, validation or
	 * destroy methods are swallowed silently.
	 * </p>
	 * @return a valid, activated object instance
	 * @throws NoSuchElementException if a valid object cannot be provided
	 * @throws IllegalStateException if invoked on a {@link #close() closed} pool
	 * @throws E if an exception occurs creating a new instance
	 */
	...
	private PooledSoftReference<T> findReference(final T obj) {
		final Optional<PooledSoftReference<T>> first = allReferences.stream()
			.filter(reference -> reference.getObject() != null && reference.getObject().equals(obj))
			.findFirst();
		return first.orElse(null);
	}

	/**
	 * Gets the {@link PooledObjectFactory} used by this pool to create and manage object
	 * instances.
	 * @return the factory
	 */
	...
	/**
	 * Gets an approximation not less than the of the number of idle instances in the
	 * pool.
	...
	/**
	 * If any idle objects were garbage collected, remove their {@link Reference} wrappers
	 ...
	 * returning instance is destroyed if any of the following are true:
	 * <ul>
	 * <li>the pool is closed</li>
	 * <li>{@link PooledObjectFactory#validateObject( org.apache.commons.pool2.PooledObject)
	 * validation} fails</li>
	 * <li>{@link PooledObjectFactory#passivateObject( org.apache.commons.pool2.PooledObject)
	 * passivation} throws an exception</li>
	 * </ul>
	 * Exceptions passivating or destroying instances are silently swallowed. Exceptions
	 * validating instances are propagated to the client.
