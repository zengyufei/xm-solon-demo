 ...
 * <p>
 * This class is currently implemented using {@link ScheduledThreadPoolExecutor}. This
 * implementation may change in any future release. This class keeps track of how many
 * pools are using it. If no pools are using the timer, it is cancelled. This prevents a
 * thread being left running which, in application server environments, can lead to memory
...
class EvictionTimer {
	 ...
	 */
	private static final HashMap<WeakReference<BaseGenericObjectPool<?, ?>.Evictor>, WeakRunner<BaseGenericObjectPool<?, ?>.Evictor>> TASK_MAP = new HashMap<>(); // @GuardedBy("EvictionTimer.class")
	 ...
	 * @param timeout If the associated executor is no longer required, how long should
	 * this thread wait for the executor to terminate?
	...
	/**
	 * Gets the task map. Keys are weak references to tasks, values are runners managed by
	 * executor.
	 * @return the task map.
	 */
	...
	/**
	 * Removes evictor from the task set and executor. Only called when holding the class
	 * lock.
	...
	/**
	 * Adds the specified eviction task to the timer. Tasks that are added with a call to
	 * this method *must* call
	 * {@link #cancel(BaseGenericObjectPool.Evictor, Duration, boolean)} to cancel the
	 * task to prevent memory and/or thread leaks in application server environments.
	 * @param task Task to be scheduled.
	 * @param delay Delay in milliseconds before task is executed.
	 * @param period Time in milliseconds between executions.
	 */
	static synchronized void schedule(final BaseGenericObjectPool<?, ?>.Evictor task, final Duration delay,
	...
	/**
	 * Thread factory that creates a daemon thread, with the context class loader from
	 * this class.
	 */
	private static class EvictorThreadFactory implements ThreadFactory {
		...
		public Thread newThread(final Runnable runnable) {
			...
			thread.setDaemon(true); // POOL-363 - Required for applications using
	...
	/**
	 * Task that removes references to abandoned tasks and shuts down the executor if
	 * there are no live tasks left.
	 */
	...
	/**
	 * Runnable that runs the referent of a weak reference. When the referent is no no
	 * longer reachable, run is no-op.
	 *
