...
/**
 * This wrapper is used to track the additional information, such as state, for the pooled
 * objects.
 * <p>
...
public class DefaultPooledObject<T> implements PooledObject<T> {
	...
	private PooledObjectState state = PooledObjectState.IDLE; // @GuardedBy("this") to
	...
	/**
	 * Creates a new instance that wraps the provided object so that the pool can track
	 * the state of the pooled object.
	 ...
	 * currently {@link PooledObjectState#ALLOCATED ALLOCATED} or
	 * {@link PooledObjectState#RETURNING RETURNING}.
	 ...
	 * detailed stack traces are required. When set to false, abandoned logs may only
	 * include caller class information rather than method names, line numbers, and other
	 * normal metadata available in a full stack trace.
	...
	public void setRequireFullStackTrace(final boolean requireFullStackTrace) {
		borrowedBy = CallStackUtils.newCallStack(
				"'Pooled object created' "
						+ "yyyy-MM-dd HH:mm:ss Z 'by the following code has not been returned to the pool:'",
				true, requireFullStackTrace);
		usedBy = CallStackUtils.newCallStack("The last code to use this object was:", false, requireFullStackTrace);
	}
